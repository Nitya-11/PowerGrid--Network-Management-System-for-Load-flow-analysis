...
export default function GridPulseDashboard() {
  ...
  const getFirstAvailableLoadDate = async () => {
    ...
    if (!loadsRes.ok) {
      ...
      throw new Error(`Loads API failed: ${loadsRes.status} ${loadsRes.statusText} - ${text || 'no response body'}`);
    }

    const loads = await loadsRes.json();
    if (!loads.length) {
      throw new Error('No load data available');
    }

    const dates = Array.from(new Set(loads.map(load => load.timestamp.split('T')[0]))).sort();
    return dates[0];
  };
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      let simDate = DATE;
      let simRes  = await fetch(`${API}/simulation/?date=${simDate}`);

      if (!simRes.ok) {
        const text = await simRes.text();

        if (simRes.status === 404) {
          simDate = await getFirstAvailableLoadDate();
          simRes = await fetch(`${API}/simulation/?date=${simDate}`);
        }
      }
      ...
      const sumRes  = await fetch(`${API}/dashboard-summary/?timestamp=${simDate}T${ts}:00`);
      if (!sumRes.ok) {
  ...
  if (error) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center",
      ...
      <div style={{ background: "#1e293b", color: "#94a3b8", padding: "12px 20px",
        ...
        curl -X POST "http://localhost:8001/api/run-simulation/?date=2026-04-15"
      </div>
          ...
          <div style={{ background: card, border: `1px solid ${border}`,
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={snapshotData} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
                ...
                <YAxis domain={[0.96, 1.04]} tick={{ fontSize: 11, fill: subtext }}
                  tickLine={false} axisLine={false} tickFormatter={v => v.toFixed(2)} />
                {showLimits && <>
                  <ReferenceLine y={LIMIT_NORMAL_HIGH} stroke="#f59e0b" strokeDasharray="4 4" strokeWidth={1} />
                  <ReferenceLine y={LIMIT_NORMAL_LOW}  stroke="#f59e0b" strokeDasharray="4 4" strokeWidth={1} />
                ...
                <Bar dataKey="vm_pu" radius={[6, 6, 0, 0]}
                  label={{
                    position:"top",
                    fill: "#cbd5e1",
                    fontSize: 11,
                 ...
                 <Tooltip
                   formatter={(value) => [value.toFixed(4), "Voltage"]}
